...
class Davinci:
    def __init__(self, filepath: str, location: str | None = None) -> None:
        ...
        self.__hsl = int((self.__long_side + 0.2 * self.__short_side) / 2)
        ...
        self.__font = ImageFont.truetype(
            "./fonts/JetBrainsMono-Medium.ttf", self.__font_size
        )
        self.__info_font = ImageFont.truetype(
            "./fonts/JetBrainsMono-LightItalic.ttf", int(self.__font_size * 0.9)
        )
        self.__artist_font = ImageFont.truetype(
            "./fonts/JetBrainsMono-LightItalic.ttf", int(self.__font_size * 1.1)
        )
    ...
    def __add_capture_time(self) -> None:
        ...
        y: int = int(self.__hsl - self.__original_height / 2 - 0.017 * self.__hsl)
    ...
    def __add_location(self) -> None:
        ...
        x: int = int(self.__hsl + self.__original_width / 2)
        y: int = int(self.__hsl - self.__original_height / 2 - 0.013 * self.__hsl)
    ...
    def __add_camera_params(self) -> None:
        ...
        focal_length: str = "Φ" + str(self.__exif["EXIF:FocalLength"]) + "mm"
        text: str = focal_length + " " + iso + " " + aperture + " " + exposure_time
        x: int = int(self.__hsl - self.__original_width / 2)
        y: int = int(
            self.__hsl
            + self.__original_height / 2
            + self.__font_size
            + 0.021 * self.__hsl
        )
        self.__draw.text((x, y), text, anchor="lt", font=self.__info_font, fill="black")

    def __add_artist(self) -> None:
        # artist, on bottom right corner
        text = str(self.__exif["EXIF:Artist"])
        x = int(self.__hsl + self.__original_width / 2 - 0.012 * self.__hsl)
        y = int(self.__hsl + self.__original_height / 2 + 0.027 * self.__hsl)
        self.__draw.text(
            (x, y), text, anchor="rt", font=self.__artist_font, fill="black"
        )
